...
class ProducerLiteConfig:
    """Producer exp48 core with threshold-protected Borg tie cognition."""
    ...
    # --- Borg assimilation layer --------------------------------------------
    # Score 800 showed that hard Borg overwrites can damage the Producer core.
    # This layer is deliberately threshold-protected: it cannot make a
    # sub-threshold attack fire. It only reorders candidates already approved by
    # the exact flow-difference scorer.
    borg_enabled: bool = True
    borg_tie_cap: float = 0.030
    borg_neutral_weight: float = 0.018
    borg_prod_weight: float = 0.006
    borg_enemy_hub_weight: float = 0.004
    borg_leader_weight: float = 0.003
    borg_speed_weight: float = 0.006
...
def _borg_strength_by_owner(obs, prod: Tensor, *, player_count: int, dtype: torch.dtype) -> Tensor:
    """Collective map: production + light garrison mass per owner."""
    device = obs.device
    count = max(1, int(player_count))
    owner = obs.owner_abs.to(torch.long)
    valid_owner = (owner >= 0) & (owner < count) & obs.alive
    owner_idx = owner.clamp(min=0, max=count - 1)
    prod_by_owner = torch.zeros(count, dtype=dtype, device=device)
    ships_by_owner = torch.zeros(count, dtype=dtype, device=device)
    prod_by_owner.scatter_add_(
        0, owner_idx, torch.where(valid_owner, prod.to(dtype), torch.zeros_like(prod.to(dtype)))
    )
    ships_by_owner.scatter_add_(
        0, owner_idx, torch.where(valid_owner, obs.ships.to(dtype), torch.zeros_like(obs.ships.to(dtype)))
    )
    return prod_by_owner + 0.025 * ships_by_owner


def borg_threshold_tiebreak(
    *,
    obs,
    prod: Tensor,
    target_idx: Tensor,
    ...
    """Tiny Borg Council directive that cannot create new launches.

    Borg behavior here means collective preference, not a planner replacement:
    assimilate weak productive neutrals, cut productive enemy hubs, pressure the
    strongest enemy in 4P, and prefer faster arrivals. The exact flow-difference
    score still decides whether a candidate is legal enough to fire.
    """
    if not bool(getattr(config, "borg_enabled", True)) or int(base_score.numel()) == 0:
        return torch.zeros_like(base_score)
    ...
    ships_t = obs.ships[tgt].to(dtype).clamp(min=0.0)
    neutral_t = obs.is_neutral[tgt] & target_exists
    enemy_t = obs.is_enemy[tgt] & target_exists
    owned_enemy_t = enemy_t & (obs.owner_abs[tgt] >= 0)

    weak_neutral_pull = (
        neutral_t.to(dtype)
        * float(config.borg_neutral_weight)
        * (prod_t + 1.0)
        / (1.0 + 0.15 * ships_t)
    )
    production_pull = (~obs.owned[tgt] & target_exists).to(dtype) * float(config.borg_prod_weight) * prod_t
    enemy_hub_pull = enemy_t.to(dtype) * float(config.borg_enemy_hub_weight) * prod_t

    leader_pull = torch.zeros_like(prod_t)
    if int(player_count) >= 4:
        strength = _borg_strength_by_owner(obs, prod, player_count=int(player_count), dtype=dtype)
        my_strength = strength[pid].detach() if pid < int(strength.shape[0]) else torch.tensor(0.0, dtype=dtype, device=device)
        owner_t = obs.owner_abs[tgt].to(torch.long).clamp(min=0, max=int(strength.shape[0]) - 1)
        leader_delta = (strength[owner_t] - my_strength).clamp(min=0.0)
        leader_pull = owned_enemy_t.to(dtype) * float(config.borg_leader_weight) * leader_delta

    target_pull = weak_neutral_pull + production_pull + enemy_hub_pull + leader_pull
    eta = cand_eta.squeeze(-1).to(dtype)
    fast_pull = float(config.borg_speed_weight) / (1.0 + eta.clamp(min=0.0))
    directive = target_pull[cand_tgt_short] + fast_pull

    # Normalize by send size very lightly so half/three-quarter fleets can win
    # near-ties when they preserve more collective mass.
    send = cand_send.squeeze(-1).to(dtype)
    send_scale = send.detach().max().clamp(min=1.0)
    preserve_pull = 0.004 * (1.0 - send / send_scale).clamp(min=0.0, max=1.0)
    directive = directive + preserve_pull

    cap = abs(float(config.borg_tie_cap))
    directive = directive.clamp(min=0.0, max=cap)

    # Critical safety: no sub-threshold launch is promoted by Borg.
    eligible = torch.isfinite(base_score) & (base_score > float(config.roi_threshold))
    return torch.where(eligible, directive, torch.zeros_like(directive))
...
def plan_lite_waves(
    *,
    ...
    score = score + borg_threshold_tiebreak(
        obs=obs,
        prod=prod,
        target_idx=target_idx,
        target_exists=target_exists,
...
CONFIG_4P = dataclasses.replace(
    ...
    borg_tie_cap=0.040,
    borg_neutral_weight=0.014,
    borg_prod_weight=0.005,
    borg_enemy_hub_weight=0.006,
    borg_leader_weight=0.004,
)
