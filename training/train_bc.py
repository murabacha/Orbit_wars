"""
Behavioral Cloning trainer with Aggressive Memory Management.
Target: Stable execution on 15GB/16GB VRAM GPUs.
"""
import argparse
import math
import os
...
def train_bc(npz_path: str, epochs: int = 5, batch_size: int = 16, lr: float = 3e-4, 
    ...
    print(f"🚀 Initializing BC Trainer [Batch Size: {batch_size}, Device: {device}, LR: {lr}]")
    if 'cuda' in device:
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            with torch.amp.autocast('cuda', enabled=scaler is not None):
                ...
                # Identify owned planets (Friendly = Channel 2)
        ...
        if total_valid > 0:
            print(f"Epoch {epoch+1:02d} | Loss: {total_loss/len(dataset):.4f} | Target Acc: {100*correct_targets/total_valid:.2f}% | Alloc Acc: {100*correct_allocs/total_valid:.2f}%")
